...
namespace NerdStore.Vendas.Domain.Tests
{
    public class PedidoTests
    {
        ...
        [Fact(DisplayName = "Adicionar Item Pedido Existente com soma de quantidades acima do permitido")]
        [Trait("Categoria", "Vendas - Pedido")]
        public void AdicionarItemPedido_QuantidadeDoItemExistenteSomadaANovaAdicaoAcimaDoPermitido_DeveLancarException()
        {
            // Arrange
            var pedido = Pedido.PedidoFactory.NovoPedidoRascunho(new Guid());
            var itemId = new Guid();
            var pedidoItem = new PedidoItem(itemId, "Produto", 1, 100);
            pedido.AdicionarItem(pedidoItem);

            var pedidoItem2 = new PedidoItem(itemId, "Produto", Pedido.MAX_UNIDADES_ITEM, 100);
        ...
        [Fact(DisplayName = "Atualizar Item Pedido que não está na lista")]
        [Trait("Categoria", "Vendas - Pedido")]
        public void AtualizarItemPedido_ItemNaoExistenteNaLista_DeveDispararException()
        {
            // Arrange
            var pedido = Pedido.PedidoFactory.NovoPedidoRascunho(Guid.NewGuid());
            var pedidoItemAtualizado = new PedidoItem(Guid.NewGuid(), "Produto", 1, 100);
